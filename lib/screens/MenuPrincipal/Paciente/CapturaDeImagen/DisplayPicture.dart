...
import 'package:skincanbe/screens/MenuPrincipal/Paciente/CapturaDeImagen/waitPrediagnostic.dart';
...
class _DisplaypictureState extends State<DisplayPicture> {
  ...
  void _realizarPeticion() async {
    ...
    try {
      ...
      final respuesta = await api_service.peticion_api(widget.imagen);
      final String claveLesion = respuesta['predicted_class'];
      final String porcentaje = respuesta['prob'];
      final String nombreLesion = _colocarNombre(claveLesion);
      ...
      final data = await lesionService.registrarLesion(
          widget.id, widget.imagen, nombreLesion, descripcion, porcentaje);
      ...
      if (data['id_lesion'] != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text("No se ha podido guardar la lesión")));
      }
    } catch (e) {
      // Manejo de excepciones para errores que puedan ocurrir al registrar la lesión

      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text("Error al registrar la lesión: ${e.toString()}")));
    }
  }
  ...
  String _colocarNombre(String clave) {
    if (clave == 'nv') {
      return "Nevo melanocitico";
    } else if (clave == 'bcc') {
